...
python2 empirical_convert_to_FVs.py ag1000g.phase1.ar3.haplotypes.2L.h5 2L 49364325
    1 5000000 5000 11 0.25 $pMisPol AOM_partial_stats.txt
    Anopheles-gambiae-PEST_CHROMOSOMES_AgamP3.accessible.fa
    anc.meru_mela.2L.fa samples_pops.txt AOM 2L.1.stats 2L.1.fvec
"""
import sys
import h5py
import allel
...
if segmentStart is not None:
    ...
    snpIndicesToKeep = [x for x in range(
        len(positions)) if segmentStart <= positions[x] <= segmentEnd]
...
subWinSize, numSubWins, unmaskedFracCutoff, pMisPol = int(
    subWinSize), int(numSubWins), float(unmaskedFracCutoff), float(pMisPol)
...
standardizationInfo = fvTools.readStatsDafsComputeStandardizationBins(
    partialStatAndDafFileName, nBins=50, pMisPol=pMisPol)
if maskFileName.lower() in ["none", "false"]:
    unmaskedFracCutoff = 1.0
    unmasked = [True]*chrLen
    sys.stderr.write(
        "Warning: a mask.fa file for the chr arm with all masked sites N'ed out is"
        "strongly recommended (pass in the reference to remove Ns at the very least)!\n")
else:
    unmasked = fvTools.readMaskDataForScan(maskFileName, chrArm)
    assert len(unmasked) == chrLen
ancArm = fvTools.readFaArm(ancestralArmFaFileName).upper()
sys.stderr.write("polarizing SNPs\n")
mapping, unmasked = fvTools.polarizeSnps(
...
sampleIndicesToKeep = [x for x in range(len(samples)) if sampleToPop.get(
    samples[x], "popNotFound!") == targetPop]
...
snpIndicesToKeep = [x for x in range(
    len(positions)) if unmasked[positions[x]-1]]
...
statNames = ["pi", "thetaW", "tajD", "thetaH", "fayWuH", "HapCount", "H1", "H12",
...
for i in ["HAF", "HAFunique", "phi", "kappa", "SFS", "SAFE"]:
    for j in ["Mean", "Median", "Mode", "Lower95%", "Lower50%", "Upper50%",
...
ihsVals = allel.stats.selection.ihs(
    haps, positions, use_threads=False, include_edges=False)
...
if nonNanCount == 0:
    ...
    ihsVals = fvTools.standardize_by_allele_count_from_precomp_bins(
        ihsVals, dafs, standardizationInfo["iHS"])
    precomputedStats["iHS"] = fvTools.windowVals(
        ihsVals, subWinBounds, positions, keepNans=False, absVal=True)
...
if nonNanCount == 0:
    ...
    nslVals = fvTools.standardize_by_allele_count_from_precomp_bins(
        nslVals, dafs, standardizationInfo["nSL"])
    precomputedStats["nSL"] = fvTools.windowVals(
        nslVals, subWinBounds, positions, keepNans=False, absVal=True)

# mispolarization information not used since no inference on which specific
# SNPs may be mispolarized, and since each block contains a relatively small
# number of SNPs, especially with respect to the mispolarization rate,
# cannot effectively use the overall mispolarization rate
...
def SAFEstats(hapsInSubWin, mappingDerivedInSubWin, dafsInSubWin):
    ...
    for i in haplotypes:
        ...
        for j in range(len(hapsInSubWin)):
            if hapsInSubWin[j][haplotypes[i][0]] == mappingDerivedInSubWin[j]:
                HAFunique[i] += len([hapsInSubWin[j][x] for x in range(
                    len(hapsInSubWin[j])) if hapsInSubWin[j][x] == mappingDerivedInSubWin[j]])
    ...
    for i in range(len(hapsInSubWin)):
        ...
        kappa[i] = len(kappa[i])/float(len(set([HAFunique[x]
                                                for x in HAFunique if HAFunique[x] != 0])))
        if dafsInSubWin[i] == 0 or dafsInSubWin[i] == 1:
            SAFE.append(0.0)
        else:
            SAFE.append(
                (phi[i]-kappa[i])/float(math.sqrt(dafsInSubWin[i]*(1-dafsInSubWin[i]))))
    statVals = {}
    quantiles = {"Lower95%": 2.5, "Lower50%": 25,
                 "Upper50%": 75, "Upper95%": 97.5}
    for i in ["HAF", "HAFunique", "phi", "kappa", "SFS", "SAFE"]:
        if i == "SFS":
        ...
        if(len(np.unique(windowStats, return_counts=True)[1]) == 1):
            ...
            if(sorted(np.unique(windowStats, return_counts=True)[1])[-1] != sorted(np.unique(windowStats, return_counts=True)[1])[-2]):
                statVals[i +
                         "-Mode"] = scipy.stats.mstats.mode(windowStats)[0][0]
            else:
                ...
                for j in range(1, 51):
                    if len([x for x in windowStats if x >= (min(windowStats)+(j*((max(windowStats)-min(windowStats))/50))) and x < (min(windowStats)+((j+1)*((max(windowStats)-min(windowStats))/50)))]) >= len([x for x in windowStats if x >= mode and x < (mode+((max(windowStats)-min(windowStats))/50))]):
                        mode = min(windowStats) + \
                            (j*((max(windowStats)-min(windowStats))/50))
                statVals[i+"-Mode"] = mode + \
                    ((max(windowStats)-min(windowStats))/100)
        for j in quantiles:
...
for subWinStart in range(int(firstSubWinStart), int(lastSubWinStart)+1, int(subWinSize)):
    ...
    unmaskedFrac = unmasked[subWinStart -
                            1:subWinEnd].count(True)/float(subWinSize)
        ...
        if unmaskedFrac < unmaskedFracCutoff:
            ...
            for statName in statNames:
                if statName != "H12" and statName != "H2/H1" and statName != "Omega" and statName != "distSkew" and statName != "distKurt" and statName[0:3] != "HAF" and statName[0:3] != "phi" and statName[0:3] != "kap" and statName[0:3] != "SFS" and statName[0:3] != "SAF":
                    if statName == "fayWuH":
                        fvTools.calcAndAppendStatValForScan(alleleCounts, positions, statName, subWinStart, subWinEnd, statVals, (len(
                            statVals["thetaH"])-1), hapsInSubWin, unmasked, precomputedStats)
                    else:
                        fvTools.calcAndAppendStatValForScan(alleleCounts, positions, statName, subWinStart,
                                                    subWinEnd, statVals, subWinIndex, hapsInSubWin, unmasked, precomputedStats)
            SAFEVals = SAFEstats(hapsInSubWin, [
                                 mapping[x][1] for x in snpIndicesInSubWins[subWinIndex]], dafs[snpIndicesInSubWins[subWinIndex]])
            for statName in SAFEVals:
                statVals[statName].append(SAFEVals[statName])
            if statFileName:
                statFile.write("\t".join([chrArm, str(subWinStart), str(
                    subWinEnd)] + [str(statVals[statName][-1]) for statName in statNames]) + "\n")
    goodSubWins = goodSubWins[1:]
    if goodSubWins.count(True) == numSubWins:
        outVec = []
        for statName in statNames:
            outVec += fvTools.normalizeFeatureVec(statVals[statName][-numSubWins:])
        midSubWinEnd = subWinEnd-(subWinSize*(numSubWins/2))
        midSubWinStart = midSubWinEnd-subWinSize+1
        fvecFile.write("\t".join([chrArm, str(int(midSubWinStart)), str(int(midSubWinEnd)), str(
            (subWinEnd-(subWinSize*numSubWins)+1))+"-"+str(subWinEnd)] + [str(x) for x in outVec]) + "\n")
