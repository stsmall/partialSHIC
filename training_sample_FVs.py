# -*- coding: utf-8 -*-
"""
python training_sample_FVs.py spNeut.fvec spHard_ spSoft_ spPartialHard_
    spPartialSoft_ 5 0,1,2,3,4,6,7,8,9,10 ./ neut.fvec,hard.fvec,linkedHard.fvec,
    soft.fvec,linkedSoft.fvec,partialHard.fvec,linkedPartialHard.fvec,
    partialSoft.fvec,linkedPartialSoft.fvec
"""
import sys
import os
import random
...
hardHeader, hardExamples = getExamplesFromFVFileLs(
    sweepFilePaths[hardTrainingFilesPrefix])
linkedHardHeader, linkedHardExamples = getExamplesFromFVFileLs(
    linkedFilePaths[hardTrainingFilesPrefix])
softHeader, softExamples = getExamplesFromFVFileLs(
    sweepFilePaths[softTrainingFilesPrefix])
linkedSoftHeader, linkedSoftExamples = getExamplesFromFVFileLs(
    linkedFilePaths[softTrainingFilesPrefix])
partialHardHeader, partialHardExamples = getExamplesFromFVFileLs(
    sweepFilePaths[partialHardTrainingFilesPrefix])
linkedPartialHardHeader, linkedPartialHardExamples = getExamplesFromFVFileLs(
    linkedFilePaths[partialHardTrainingFilesPrefix])
partialSoftHeader, partialSoftExamples = getExamplesFromFVFileLs(
    sweepFilePaths[partialSoftTrainingFilesPrefix])
linkedPartialSoftHeader, linkedPartialSoftExamples = getExamplesFromFVFileLs(
    linkedFilePaths[partialSoftTrainingFilesPrefix])
...
trainingSetLs = [hardExamples, linkedHardExamples, softExamples, linkedSoftExamples,
                 partialHardExamples, linkedPartialHardExamples, partialSoftExamples, linkedPartialSoftExamples]
numExamplesToKeep = getMinButNonZeroExamples(trainingSetLs)
for i in list(range(len(trainingSetLs))):
...
if sampledFVsFiles.lower() in ["none", "false", "default"]:
    sampledFVsFiles = ["neut.fvec", "hard.fvec", "linkedHard.fvec", "soft.fvec", "linkedSoft.fvec",
...
outExamples = [neutExamples, hardExamples, linkedHardExamples, softExamples, linkedSoftExamples,
               partialHardExamples, linkedPartialHardExamples, partialSoftExamples, linkedPartialSoftExamples]
for i in list(range(len(sampledFVsFiles))):
    if outExamples[i]:
        outFile = open(
            '/'.join((sampledFVsDir+'/'+sampledFVsFiles[i]).split('//')), "w")
