...
python training_convert_to_FVs.py spNeut.msOut.gz 2L,2R,3L,3R 5000 11 0.25
    $pMisPol AOM_partial_stats.txt Anopheles-gambiae-PEST_CHROMOSOMES_AgamP3.accessible.fa
    anc.meru_mela.fa ./ spNeut.msOut.fvec
"""
import sys
import msTools
import fvTools
...
if maskFileName.lower() in ["none", "false"]:
    ...
    sys.stderr.write(
        "Warning: not doing any masking! (i.e. mask.fa file for the chr arm with"
        "all masked sites N'ed out, or at least the reference with Ns, is not provided)\n")
    maskFileName = False
else:
    if ancestralArmFaFileName.lower() in ["none", "false"]:
        maskData = fvTools.readMaskDataForTraining(
            maskFileName, totalPhysLen, subWinSize, chrArmsForMasking, shuffle=True, cutoff=unmaskedFracCutoff)
    else:
        maskData = fvTools.readMaskAndAncDataForTraining(
            maskFileName, ancestralArmFaFileName, totalPhysLen, subWinSize, chrArmsForMasking, shuffle=True, cutoff=unmaskedFracCutoff)
...
for subWinIndex in range(numSubWins):
    statFileName = '/'.join(("%s/%s.subWin%d.stats" % (statDir, trainingDataFileName.split(
        "/")[-1].replace(".msOut.gz", ""), subWinIndex)).split('//'))
    statFiles.append(open(statFileName, "w"))
    statFiles[-1].write("\t".join(statNames)+"\n")
if fvecFileName.lower() in ["none", "false", "default"]:
    fvecFileName = "%s.fvec" % trainingDataFileName.split(
        "/")[-1].replace(".msOut.gz", "")
...
for instanceIndex in range(len(hapArraysIn)):
    ...
    snpIndicesToKeep = [x for x in range(len(
        positionArrays[instanceIndex])) if unmasked[positionArrays[instanceIndex][x]-1]]
    if len(snpIndicesToKeep) == 0:
        for subWinIndex in range(numSubWins):
            for statName in statNames:
                fvTools.appendStatValsForMonomorphic(
                    statName, statVals, instanceIndex, subWinIndex)
            statFiles[subWinIndex].write("\t".join(
                [str(statVals[statName][instanceIndex][subWinIndex]) for statName in statNames])+"\n")
    else:
        haps = allel.HaplotypeArray(
            hapArraysIn[instanceIndex], dtype='i1').subset(sel0=snpIndicesToKeep)
        if pMisPol > 0:
            misPolarizeCorrectionIndex = np.random.binomial(
                1, pMisPol, len(haps))
        ...
        positions = [positionArrays[instanceIndex][x]
                     for x in snpIndicesToKeep]
        precomputedStats = {}
        ...
        for subWinIndex in range(numSubWins):
            ...
            if len(snpIndicesInSubWins[subWinIndex]) == 0:
                ...
                hapsInSubWin = haps.subset(
                    sel0=snpIndicesInSubWins[subWinIndex])
                for statName in [x for x in statNames if x[0:3] != "HAF" and x[0:3] != "phi" and x[0:3] != "kap" and x[0:3] != "SFS" and x[0:3] != "SAF"]:
                    fvTools.calcAndAppendStatVal(alleleCounts, positions, statName, subWinStart, subWinEnd,
                                                 statVals, instanceIndex, subWinIndex, hapsInSubWin, unmasked, precomputedStats)
                haplotypes = {}
                for i in range(len(hapsInSubWin[0])):
                    haplotype = [hapsInSubWin[x][i]
                                 for x in range(len(hapsInSubWin))]
                ...
                for i in haplotypes:
                    ...
                    for j in range(len(hapsInSubWin)):
                        if hapsInSubWin[j][haplotypes[i][0]] == 1:
                            HAFunique[i] += sum([hapsInSubWin[j][x]
                                                 for x in range(len(hapsInSubWin[j]))])
                ...
                for i in range(len(hapsInSubWin)):
                    ...
                    phiDenom = 0
                    for j in haplotypes:
                        phi[i] += (int(list(j)[i])*HAFunique[j]
                                   * len(haplotypes[j]))
                    ...
                    if dafs[snpIndicesInSubWins[subWinIndex]][i] == 0 or dafs[snpIndicesInSubWins[subWinIndex]][i] == 1:
                        ...
                        SAFE.append((phi[i]-kappa[i])/float(math.sqrt(
                            dafs[snpIndicesInSubWins[subWinIndex]][i]*(1-dafs[snpIndicesInSubWins[subWinIndex]][i]))))
                for i in ["HAF", "HAFunique", "phi", "kappa", "SFS", "SAFE"]:
                    if i == "SFS":
                        ...
                        windowStats = eval(i)
                    statVals[i +
                             "-Mean"][instanceIndex].append(np.mean(windowStats))
                    statVals[i +
                             "-Median"][instanceIndex].append(np.median(windowStats))
                    if(len(np.unique(windowStats, return_counts=True)[1]) == 1):
                        statVals[i +
                                 "-Mode"][instanceIndex].append(windowStats[0])
                    else:
                        if(sorted(np.unique(windowStats, return_counts=True)[1])[-1] != sorted(np.unique(windowStats, return_counts=True)[1])[-2]):
                            statVals[i+"-Mode"][instanceIndex].append(
                                scipy.stats.mstats.mode(windowStats)[0][0])
                        else:
                            ...
                            for j in range(1, 51):
                                if len([x for x in windowStats if x >= (min(windowStats)+(j*((max(windowStats)-min(windowStats))/50))) and x < (min(windowStats)+((j+1)*((max(windowStats)-min(windowStats))/50)))]) >= len([x for x in windowStats if x >= mode and x < (mode+((max(windowStats)-min(windowStats))/50))]):
                                    mode = min(
                                        windowStats)+(j*((max(windowStats)-min(windowStats))/50))
                            statVals[i+"-Mode"][instanceIndex].append(
                                mode+((max(windowStats)-min(windowStats))/100))
                    for j in quantiles:
                        statVals[i+"-"+j][instanceIndex].append(
                            np.percentile(windowStats, quantiles[j]))
                    statVals[i+"-Max"][instanceIndex].append(max(windowStats))
                    statVals[i +
                             "-Var"][instanceIndex].append(np.var(windowStats))
                    statVals[i +
                             "-SD"][instanceIndex].append(np.std(windowStats))
                    statVals[i+"-Skew"][instanceIndex].append(
                        scipy.stats.skew(windowStats))
                    statVals[i+"-Kurt"][instanceIndex].append(
                        scipy.stats.kurtosis(windowStats))
            statFiles[subWinIndex].write("\t".join(
                [str(statVals[statName][instanceIndex][subWinIndex]) for statName in statNames])+"\n")
    outVec = []
    for statName in statNames:
        outVec += fvTools.normalizeFeatureVec(statVals[statName][instanceIndex])
