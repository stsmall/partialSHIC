...
for testSetFileName in os.listdir(fvecDir):
    ...
    for testExample in currTestData:
        if "nan" not in testExample:
            testData = testExample.strip().split("\t")
            currVector = []
            for i in range(len(testData)):
                currVector.append(float(testData[i]))
            testX[testSetFileName].append(currVector)
    testX[testSetFileName] = np.reshape(np.array(testX[testSetFileName]), (np.array(
        testX[testSetFileName]).shape[0], int(numSumStatsPerSubWin), numSubWins, 1))

selVals = {"Neutral": 0, "Hard": 1, "Soft": 2,
           "HardPartial": 3, "SoftPartial": 4}
...
labelToClassName = {0: "Neutral", 1: "Hard", 2: "Hard-linked", 3: "Soft", 4: "Soft-linked",
                    5: "HardPartial", 6: "HardPartial-linked", 7: "SoftPartial", 8: "SoftPartial-linked"}
outlinesH = {}
accuracyOverall = 0
accuracySpecific = 0
accuracyBroad = 0
for testSetFileName in sorted(testX, key=lambda x: (selVals[getSelType(x)], int((x.split(".")[0]+"_0").split("_")[1]))):
    ...
    if not testSetFileName == 'spNeut.fvec':
        ...
        selType, selWin = getSelType(
            testSetFilePrefix[0]), testSetFilePrefix[1]
        selWin = int(selWin)
        key = (selType, selWin)
    else:
        key = ('Neutral', 0)
    outlinesH[key] = (testSetFileName, [currPreds[className]
                                        for className in classOrder])
...
fileName = open(
    '/'.join((resultsDir+'/'+accuracyFilesPrefix+'_overall.txt').split('//')), 'w')
fileName.write(str(accuracyOverall))
fileName.close()
accuracySpecific = accuracySpecific/5
fileName = open(
    '/'.join((resultsDir+'/'+accuracyFilesPrefix+'_specific.txt').split('//')), 'w')
fileName.write(str(accuracySpecific))
fileName.close()
accuracyBroad = accuracyBroad/((4*numSubWins)+1)
fileName = open(
    '/'.join((resultsDir+'/'+accuracyFilesPrefix+'_broad.txt').split('//')), 'w')
            ...
            if "Neutral" in selType:
                ...
                if selWin == 5:
                    ...
                    if selWin < 5:
                        ...
                        direction = "right"
                    rowLabels.append("%s sweep %s window%s to %s" %
                                     (selType, diff, plural, direction))
...
cbar.set_label('Fraction of simulations assigned to class',
...
for y in range(data.shape[0]):
    for x in range(data.shape[1]):
        ...
        if val > 50:
            ...
            c = 'black'
        ax.text(x+0.5, y + 0.5, '%.1f%%' % val, horizontalalignment='center',
                verticalalignment='center', color=c, fontsize=6)
if confusionMatrixFigFileName.lower() in ["none", "false", "default"]:
    confusionMatrixFigFileName = 'confusionmatrix.pdf'
plt.savefig('/'.join((resultsDir+'/'+confusionMatrixFigFileName).split('//')),
